...
static int xwifi_nl80211_print_event(struct nl_msg *msg, void *arg)
{
	S_;
	struct genlmsghdr *gnlh = nlmsg_data(nlmsg_hdr(msg));
	struct nlattr *tb[NL80211_ATTR_MAX + 1], *nst;
...
int xwifi_nl80211_prepare_listen_events(struct nl80211_state *state)
{
	S_;
	int mcid, ret;
			 ...
			 struct print_event_args *args)
{
	...
	if (n_waits && waits) {
		...
		nl_cb_set(cb, NL_CB_VALID, NL_CB_CUSTOM, xwifi_nl80211_print_event, args);
...
__u32 xwifi_nl80211_listen_events(struct nl80211_state *state,
			 const int n_waits, const __u32 *waits,
			 struct print_event_args *args)
{
	S_;
...
__u32 xwifi_listen_events(struct nl80211_state *state, const int n_waits, const __u32 *waits)
{
	S_;
	int ret;
