...
static int xwifi_event_queue_server_initialise(struct xwifi_event_queue* event_queue, const char* file);
...
int xwifi_close_event_queue(const int fd)
{
	...
	osl_mutex_close(event_queue->lck);
	free(event_queue);
	return 0;
}
...
//		pthread_mutex_init(&event_queue->lck, NULL);
		...
		xwifi_event_queue_server_initialise(event_queue, UNIX_DOMAIN_FILE_PATH);
	}
	return event_queue;
}

static int xwifi_event_queue_server_initialise(struct xwifi_event_queue* self, const char* file)
{
	S_;
